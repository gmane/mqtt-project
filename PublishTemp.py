...
def main():
	...
	while 0 < 1:
		...
		thime = parseTime(rep)
		if thime != lasttime:
			publish.single("/SanAntonioTemp", payload=("At "+str(thime)+" the temp was "+str(temp)),qos=2,hostname="192.168.2.63")
			...
			count = count + 15
			publish.single("/SanAntonioTemp", payload=("Time last updated was "+str(count/60)+ " minutes ago and the temperature was "+str(temp)),qos=2,hostname="192.168.2.63")
